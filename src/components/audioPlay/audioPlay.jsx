...
class AudioPlay extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      //用户收藏
      music_like: storage.getUser().music_like ? storage.getUser().music_like : []
    };
  }
  // 下方控制
  lock = () => {
    if (this.state.islock === "icon-suo1") {
      this.setState({
        islock: "icon-suo",
        mainLayout: "mainLayout-lock"
      })
    } else {
      this.setState({
        islock: "icon-suo1",
  ...
  UNSAFE_componentWillReceiveProps(props) {
    const musicList = props.playerList
    this.setState({
      musicList: musicList,
      music_like: storage.getUser().music_like ? storage.getUser().music_like : []
    })
    if (parseInt(props.isplay)) {
      var currentMusic = {}
  ...
  onAddFile = async(e, item) => {
    ...
    if(result.status===0){
      ...
      this.setState({music_like:storage.getUser().music_like })
    }else{
  ...
  onShareMusic = (e, item) => {
    e.stopPropagation();
    alert(
      `分享指定歌曲，尚未完善，歌曲id:${item.id}，歌曲名称:${item.title}`
    );
  };
  ...
  onUploadMusic = (e, item) => {
    e.stopPropagation();
    alert(
      `下载指定歌曲，尚未完善，歌曲id:${item.id}，歌曲名称:${item.title}`
    );
  };
  ...
  onDeleteMusic = (e, item) => {
    e.stopPropagation();
    const newMusicList = this.props.deleteOnemusic(item.id, this.props.playerList)
    this.setState({
  ...
  render() {
    ...
    return (
      <div className={mainLayout}>
        ...
        <div>
          <div className="playerlock iconfont icon-tixing ">
            <div className={"locked iconfont " + islock} onClick={this.lock}>
            </div>
          </div>
        </div>
        <div
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="col music-list-li-col-3" >
                                <span
                                  className={music_like.includes(item.id.toString()) ? "iconfont icon-xihuan2" : "iconfont icon-xihuan1"}
